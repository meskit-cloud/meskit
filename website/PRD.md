...
MESkit is an open-source MES toolkit with built-in analytics that is:

- ISA-95 aligned.
- Smart features built in — quality alerts, production planning, natural language queries.
- Simulation-first.
...
Every MES operation flows through a **tool layer** consumed by both the UI and smart features. The same typed function that powers a button click also powers a natural-language command. MESkit ships with three smart features:
...
MVP product scope focuses on discrete manufacturing UX (Build, Configure, Run, Monitor) with an always-available chat panel for natural language interaction. Schema supports batch and continuous production types.

## 3. Website Goals

### Primary goals

1. Position MESkit as an open-source MES with built-in analytics and natural language queries.
...
1. Build topical authority around ISA-95 + open-source MES + smart manufacturing + MQTT integration.
2. Create reusable, citation-friendly content for AI assistants.
3. Establish MESkit as the reference for open-source MES with built-in analytics.
...
See problems before they stop your line. MESkit monitors machine health, surfaces quality trends, and helps you plan production — so your team acts on insights instead of chasing data. Three automation layers build toward this: the Monitor (Machine Health Monitor) detects degradation, the Planner evaluates alternatives, and the intelligence layer acts through the tool layer — operators stay in command.
...
1. **Unified tool layer**: Every MES operation is a typed tool that both the UI and smart features call — removing coordination bottlenecks so operators focus on decisions.
...
5. **Three smart features**: Ask MESkit (natural language), Quality Monitor (event-driven alerts), Production Planner (on-demand) — each with defined tools, triggers, and roles.
6. **Monitor → Plan → Act**: Three automation layers designed to close the coordination loop from detection to rescheduling — the system handles the chain, operators approve the decisions.

### Messaging guardrails

1. Do not imply batch/continuous UI is fully available in MVP.
2. Do not position smart features as autonomous decision-makers — they operate through the same tool layer as humans, with human oversight. Smart features remove coordination bottlenecks; operators remain the decision-makers. The North Star can be referenced as the product direction, clearly distinguished from current capabilities.
3. Avoid generic "AI manufacturing" or "AI-powered" claims without specifying what smart features actually do (which tools they call, what triggers them). Lead with outcomes (dashboards, alerts, predictions, planning, natural language).
4. Prioritize concrete technical language over vague marketing terms.
5. The underlying intelligence layer uses LLMs for tool-use, but buyer-facing copy should describe capabilities (quality alerts, natural language queries, production planning), not technology.
...
| `--agent` | `#7C3AED` | `violet-600` | Smart feature badges and chat/example accents |
...
3. **Smart feature violet** is reserved exclusively for content about smart features (chat examples, feature cards, smart features page accents). Do not use it as a general-purpose accent.
...
| **OG image** | Light gradient background with white card, centered MESkit wordmark, tagline below: "Open-source MES with built-in analytics." 1200x630px. |
...
| **Concrete** | Every claim has a specific mechanism. | "The Quality Monitor calls `get_yield_report` when yield drops below 90%." — not "AI monitors your quality." |
| **Confident, not hype** | State what it does. Don't oversell. | "smart architecture" — not "revolutionary AI breakthrough" |

#### Tone by context

| Context | Tone | Example |
|---------|------|---------|
| Hero / homepage | Bold, declarative | "Finally, an MES that's as easy to use as asking a question." |
| Product pages | Explanatory, specific | "Each smart feature calls typed tools through the same interface the UI uses." |
...
| CTAs | Action-oriented, lowercase verbs | "View on GitHub" / "Read the docs" / "See smart features" |
...
5. Use **active voice**. "MESkit calls `move_unit`" — not "The `move_unit` function is called by the system."
...
- Syntax highlighting: muted palette — blue for keywords, green for strings, violet for smart-feature-related terms, slate for comments
- Top bar with language label and optional copy button

#### Chat examples

When showing MESkit interactions on the website (for illustration, not live):

- User message: `--surface` background, aligned right, `--text-primary` text
- MESkit message: `--surface` background with subtle `--agent` left border (3px), aligned left
- Tool call indicator: monospace `--text-tertiary` text showing tool name (e.g., `→ get_wip_status()`)
- Result: `--text-secondary` in the MESkit response
...
1. `/` Home
2. `/product` Product overview (modes + chat panel + smart features)
...
3. `/glossary/{term}`
4. `/agents/{feature-name}` (deep-dive per smart feature)
...
2. "How it works" flow aligned to product loop:
   ...
   - Visualize.
   - ...or ask MESkit.
3. Natural language showcase — brief visual of the chat panel with example interactions (e.g., "Scrap SMX-00044, solder defect" → MESkit calls tools → result).
4. Tool layer concept — one diagram showing UI and smart features calling the same tools.
...
### Smart Features (`/agents`)

Must include:

1. What MESkit's smart features are — tool layer architecture, not a bolted-on chatbot.
2. Three smart feature profiles:
   - **Ask MESkit**: always-on chat, example interactions, available tools.
   - **Quality Monitor**: event-driven triggers, alert examples, monitoring thresholds.
   - **Production Planner**: on-demand planning, capacity analysis examples.
3. Tool layer explanation — how the same typed functions serve UI buttons and natural language.
4. Architecture diagram (intelligence layer → tool layer → Supabase).
5. Concrete examples with tool call chains (user says X → MESkit calls Y → result Z).
6. Guardrails: smart features operate through the tool layer with human oversight — they remove coordination bottlenecks, not human decision-making.
...
3. Note on how smart features operate at ISA-95 Level 3 — same level as human operators.
4. Internal links to architecture, smart features, and docs.

### Architecture (`/architecture`)

Must include:

1. Four-layer design diagram (Frontend → Tool Layer → Intelligence Layer → Supabase → MQTT).
2. Tool layer rationale — single source of truth for UI and smart features.
3. Intelligence layer explanation — Claude API tool-use, streaming, system prompts.
...
1. Milestone timeline (M1-M6) with smart feature deliverables highlighted per milestone.
2. Explicit status labels (planned, in progress, completed).
3. Feature progression: M1 (scaffold) → M2 (Ask MESkit) → M4 (Quality Monitor) → M5 (Planner) → M6 (Machine Health Monitor).
...
1. Category taxonomy: ISA-95, MES architecture, smart features, simulation, MQTT, Supabase.
2. Author, publish date, update date.
3. Structured data and internal links to product pages.

Priority launch content:
- "Why your MES needs a universal tool layer" (category-defining explainer)
- "How MESkit smart features call MES tools" (technical deep-dive on tool layer)
...
3. **MES with analytics / smart MES**: "MES with analytics", "MES natural language", "smart manufacturing MES", "MES with AI" → `/agents`
...
1. Every page links upward and laterally (home, docs, architecture, smart features, roadmap).
2. Blog posts must link to at least one product page and one docs page.
3. The `/agents` (smart features) page must be linked from home, product, architecture, and FAQ.
...
1. Entity clarity:
   ...
   - Clearly define "open-source MES with built-in analytics" as MESkit's category.
2. Fact consistency:
   - Keep one source-of-truth facts block (stack, standards, scope, smart features, roadmap status).
...
1. Create a "Core Facts" block on Home and FAQ with:
   - Product category (open-source MES toolkit with built-in analytics).
   - Open-source status.
   - ISA-95 alignment.
   - Smart features architecture (tool layer, three smart features).
   - MVP scope status.
...
2. Maintain an AI-friendly FAQ page targeting prompts like:
   ...
   - "How does the MESkit natural language interface work?"
...
4. Keep terminology stable across pages:
   - `tool layer`, `Ask MESkit`, `Quality Monitor`, `Production Planner`
...
2. Inconsistent claims across pages (for example, saying batch UI is already shipped, or implying smart features are autonomous without human oversight).
3. Long paragraphs without scannable structure.
4. No source links for critical claims.
5. Generic "AI-powered" claims without specifying what smart features do and which tools they call.
...
6. Smart feature claims verified against current product PRD (which features exist, what tools they have, what triggers them).
...
2. Impressions and CTR for priority queries (including "AI MES" and "smart MES" clusters).
...
4. Smart features page (`/agents`) engagement (time on page, scroll depth, outbound clicks to docs/GitHub).
...
2. Live demo or interactive chat on the website (the smart features run inside the MESkit app, not on the marketing site).
...
1. All required MVP pages are published and internally linked (including `/agents` smart features page).
2. Metadata, canonical tags, sitemap, robots, and JSON-LD are implemented.
3. Home, product, smart features, ISA-95, architecture, and FAQ each contain answer-ready GEO blocks.
...
7. Smart feature descriptions on the website match the current product PRD (tools, triggers, roles).
...
4. Product PRD as single source of truth for smart feature capabilities and tool catalog.

### Risks

1. Product terminology drift between app/docs/website can harm SEO and GEO trust — especially for core terms (tool layer, intelligence layer, smart features).
2. Shipping thin content pages can reduce ranking and citation quality.
3. Overpromising smart feature capabilities can increase bounce and reduce credibility — always tie claims to specific tools and triggers.
4. "Built-in analytics" and "natural language" positioning must be substantiated with concrete architecture details to avoid being perceived as marketing hype.
